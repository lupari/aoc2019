...
object Day11b extends Challenge {
  ...
  def paint(program: ic.Program): Grid[Char] = {
    ...
    val grid: Grid[Char] = Map(Point(0, 0) -> '#').withDefaultValue('.')
    acc(grid, Dir(Point(0, 0), 'U'), Nil, ic.Input(program, List(1), Some(ic.State(0, 0))))
  }

  override def run(): Any = {
    val program = ic.read(Source.fromResource("day11.txt"))
    paint(program).canvas(' ')(c => if (c == '#') '█' else ' ')
  }
